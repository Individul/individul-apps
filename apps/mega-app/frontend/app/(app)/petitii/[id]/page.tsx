...
export default function PetitionDetailPage() {
  ...
  useEffect(() => {
    ...
    petitionsApi.get(token, petitionId)
      ...
      .catch(() => {
        toast.error('Nu s-a putut incarca petitia')
        router.push('/petitii')
      })
  ...
  const handleSaveStatus = async () => {
    ...
    try {
      ...
      await refreshPetition(token, petition.id)
      setEditingStatus(false)
      toast.success('Petitia a fost actualizata')
    } catch (error) {
      if (error instanceof ApiError) {
        toast.error(error.message)
      } else {
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setPetition(updated)
      toast.success('Fisierul a fost incarcat')
    } catch (error) {
      if (error instanceof ApiError) {
        toast.error(error.message)
      } else {
        toast.error('A aparut o eroare la incarcare')
      }
    } finally {
      setUploading(false)
  ...
  const handleDownloadAttachment = async (attachmentId: string, filename: string) => {
    ...
    try {
      const response = await fetch(`${API_URL}/api/v1/petitions/attachments/${attachmentId}/download/`, {
        headers: {
      ...
      toast.error('A aparut o eroare la descarcare')
    }
  }
  ...
  const handleDeleteAttachment = async (attachmentId: string) => {
    ...
    if (!confirm('Sigur doriti sa stergeti acest fisier?')) return
  ...
  const handleFinalize = async () => {
    ...
    if (!confirm('Sigur doriti sa marcati petitia ca finalizata?')) return
    ...
    try {
      ...
      await refreshPetition(token, petition.id)
      toast.success('Petitia a fost marcata ca finalizata')
    } catch (error) {
      if (error instanceof ApiError) {
        toast.error(error.message)
      } else {
  ...
  const handleDeletePetition = async () => {
    ...
    if (!confirm(`Sigur doriti sa stergeti petitia ${petition.registration_number}? Aceasta actiune este ireversibila.`)) return

    setDeleting(true)
    try {
      await petitionsApi.delete(token, petition.id)
      toast.success('Petitia a fost stearsa')
      router.push('/petitii')
    } catch (error) {
      if (error instanceof ApiError) {
        toast.error(error.message)
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Inregistrata la {formatDate(petition.registration_date)}
            </p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          ...
          {petition.is_due_soon && !petition.is_overdue && (
            <Badge variant="warning" className={headerActionClass}>
              {petition.days_until_due} zile ramase
            </Badge>
          )}
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between">
            <CardTitle className="text-lg">Detalii petitie</CardTitle>
            {!editingDetails ? (
              <Button
                variant="outline"
                size="sm"
          ...
          <CardContent className="space-y-4">
            <div>
              <Label className="text-muted-foreground">Tip petitionar</Label>
              {editingDetails ? (
            ...
            <div>
              <Label className="text-muted-foreground">Nume petitionar</Label>
              {editingDetails ? (
                <Input
            ...
            {(editingDetails || petition.detainee_fullname) && (
              <div>
                <Label className="text-muted-foreground">Nume detinut</Label>
                {editingDetails ? (
                  <Input
            ...
            <div>
              <Label className="text-muted-foreground">Sector detentie</Label>
              {editingDetails ? (
            ...
            <div>
              <Label className="text-muted-foreground">Termen raspuns</Label>
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <CardTitle className="text-lg">Fisiere atasate</CardTitle>
          <div>
            <input
              ref={fileInputRef}
              type="file"
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        ...
                        Incarcat de {attachment.uploaded_by_name} la {formatDateTime(attachment.uploaded_at)}
                      </p>
                    </div>
                  </div>
                  <div className="flex gap-2">
