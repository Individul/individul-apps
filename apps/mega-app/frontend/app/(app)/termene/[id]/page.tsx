    ...
    <div className="bg-white rounded-lg border border-gray-200 shadow-sm overflow-hidden">
      ...
      <div className="bg-slate-50 p-5">
        ...
        <div className="grid grid-cols-3 gap-6">
          <div>
            <p className="text-[10px] uppercase tracking-wide text-slate-500 font-bold mb-1">Inceput</p>
            <p className="text-sm font-mono text-slate-700 tabular-nums">{formatDate(sentence.start_date)}</p>
          </div>
          <div>
            <p className="text-[10px] uppercase tracking-wide text-slate-500 font-bold mb-1">Sfarsit</p>
            {hasDeductions ? (
              <div>
          ...
          <div>
            <p className="text-[10px] uppercase tracking-wide text-slate-500 font-bold mb-1">Infractiune Grava</p>
      ...
      <div className="bg-white">
        ...
        <div className="grid grid-cols-[70px_1fr_120px_100px] px-5 py-3 border-b border-gray-100 bg-white">
          <span className="text-xs uppercase text-gray-400 font-semibold">Fractie</span>
        ...
        {sentence.fractions.map((fraction, index) => (
          ...
          >
            ...
            <span className={`text-sm font-medium ${fraction.days_until < 0 ? 'text-red-600' : 'text-slate-600'}`}>
              {fraction.is_fulfilled ? '—' : fraction.days_until < 0 ? `${Math.abs(fraction.days_until)} depasit` : `${fraction.days_until} ramase`}
            </span>
...
export default function PersonDetailPage() {
  ...
  const fetchPerson = async () => {
    ...
    try {
      ...
      console.error(error)
      toast.error('Nu s-a putut incarca persoana')
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const handleAddSentence = async () => {
    ...
    if (newSentence.sentence_years === 0 && newSentence.sentence_months === 0 && newSentence.sentence_days === 0) {
      toast.error('Trebuie specificata o durata a pedepsei')
      return
    }
    ...
    try {
      ...
      toast.success('Sentinta a fost adaugata cu succes')
  ...
  const handleDeletePerson = async () => {
    if (!confirm('Sigur doriti sa stergeti aceasta persoana? Toate sentintele vor fi sterse.')) {
      return
    }
    ...
    try {
      ...
      toast.success('Persoana a fost stearsa')
      router.push('/termene')
    } catch (error) {
      toast.error('A aparut o eroare')
    }
  ...
  const handleEditPerson = async () => {
    ...
    if (!editData.first_name.trim() || !editData.last_name.trim()) {
      toast.error('Numele si prenumele sunt obligatorii')
      return
    }
    ...
    try {
      ...
      if (error instanceof ApiError) {
        ...
        toast.error('A aparut o eroare la actualizare')
      }
    } finally {
      setIsEditing(false)
    }
  ...
  const handleEditSentence = async () => {
    ...
    if (!editSentenceStartDate) {
      toast.error('Data inceputului de termen este obligatorie')
      return
    }

    if (editSentenceData.sentence_years === 0 && editSentenceData.sentence_months === 0 && editSentenceData.sentence_days === 0) {
      toast.error('Pedeapsa trebuie sa aiba cel putin o zi')
      return
    }
    ...
    try {
      ...
      toast.success('Sentinta a fost actualizata cu succes')
  ...
  const handleReleasePerson = async () => {
    ...
    if (!releaseType) {
      toast.error('Selecteaza modalitatea eliberarii')
      return
    }
  ...
  const handleAddReduction = async () => {
    ...
    if (newReduction.reduction_years === 0 && newReduction.reduction_months === 0 && newReduction.reduction_days === 0) {
      toast.error('Trebuie specificata o durata a reducerii')
      return
    }
    ...
    try {
      ...
      toast.success('Reducerea a fost adaugata')
  ...
  const handleDeleteReduction = async (sentenceId: string, reductionId: string) => {
    if (!confirm('Sigur doriti sa stergeti aceasta reducere? Fractiile vor fi recalculate.')) {
      return
    }
    ...
    try {
      ...
      toast.error('A aparut o eroare la stergerea reducerii')
    }
  }
  ...
  const handleAddZpm = async () => {
    ...
    if (!zpmMonth || !zpmDays) {
      toast.error('Completeaza luna si numarul de zile')
      return
    }

    const daysValue = parseFloat(zpmDays)
    if (isNaN(daysValue) || daysValue <= 0) {
      toast.error('Numarul de zile trebuie sa fie mai mare decat 0')
      return
    }
    ...
    try {
      ...
      toast.success('ZPM a fost adaugat')
  ...
  const handleDeleteZpm = async (sentenceId: string, zpmId: string) => {
    if (!confirm('Sigur doriti sa stergeti acest ZPM?')) return
    const token = localStorage.getItem('access_token')
    if (!token) return

    try {
      await sentencesApi.deleteZpm(token, sentenceId, zpmId)
      toast.success('ZPM a fost sters')
      fetchPerson()
    } catch (error) {
      toast.error('A aparut o eroare la stergerea ZPM')
    }
  }
    ...
    return (
      <div className="flex items-center justify-center h-64">
        <div className="flex items-center gap-2 text-slate-400">
          ...
          <span className="text-sm">Se incarca...</span>
        </div>
      </div>
    )
  }
  ...
  if (!person) {
    return (
      <div className="flex flex-col items-center justify-center h-64">
        ...
        <p className="text-sm text-slate-500 mb-4">Persoana nu a fost gasita</p>
        <Link
          href="/termene"
    ...
    <div className="space-y-8">
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500 mt-0.5">
              {person.active_sentences_count} {person.active_sentences_count === 1 ? 'sentinta activa' : 'sentinte active'}
            </p>
          </div>
        </div>
        <div className="flex items-center gap-2">
        ...
        <input
          ...
          onChange={async (e) => {
            ...
            try {
              ...
              toast.success(newValue ? 'Instiintare MAI activata' : 'Instiintare MAI dezactivata')
            } catch (error) {
      ...
      <div className="space-y-5">
        <div className="flex items-center justify-between">
          <h2 className="text-sm font-semibold text-slate-800">Dosare Sentinte</h2>
          <Sheet open={sentenceFormOpen} onOpenChange={setSentenceFormOpen}>
            ...
            <SheetContent hideCloseButton className="flex flex-col p-0">
              ...
              <div className="flex items-center justify-between p-6 border-b border-gray-100">
                <div>
                  <h2 className="text-lg font-semibold text-slate-800">Adauga Sentinta</h2>
                  <p className="text-sm text-slate-500 mt-0.5">Configureaza detaliile sentintei</p>
                </div>
                <SheetClose asChild>
        ...
        {person.sentences.length === 0 ? (
          <div className="bg-white rounded-lg border border-gray-200 p-12 text-center">
            ...
            <h3 className="text-base font-medium text-slate-800 mb-1">Nicio sentinta</h3>
            <p className="text-sm text-slate-500">Nu exista sentinte inregistrate pentru aceasta persoana</p>
          </div>
        ) : (
          <div className="space-y-5">
      ...
      <Sheet open={editFormOpen} onOpenChange={setEditFormOpen}>
        <SheetContent hideCloseButton className="flex flex-col p-0">
          ...
          <div className="flex items-center justify-between p-6 border-b border-gray-100">
            <div>
              <h2 className="text-lg font-semibold text-slate-800">Editeaza Persoana</h2>
              <p className="text-sm text-slate-500 mt-0.5">Modifica datele personale</p>
            </div>
            <SheetClose asChild>
              <button className="flex items-center justify-center w-8 h-8 rounded-full text-gray-400 hover:text-gray-600 hover:bg-gray-100 transition-colors">
                <X className="h-4 w-4" strokeWidth={2} />
      ...
      <Sheet open={editSentenceFormOpen} onOpenChange={setEditSentenceFormOpen}>
        <SheetContent hideCloseButton className="flex flex-col p-0">
          ...
          <div className="flex items-center justify-between p-6 border-b border-gray-100">
            <div>
              <h2 className="text-lg font-semibold text-slate-800">Editeaza Sentinta</h2>
              <p className="text-sm text-slate-500 mt-0.5">Modifica durata, inceputul termenului si statusul</p>
            </div>
            <SheetClose asChild>
              <button className="flex items-center justify-center w-8 h-8 rounded-full text-gray-400 hover:text-gray-600 hover:bg-gray-100 transition-colors">
                <X className="h-4 w-4" strokeWidth={2} />
      ...
      <Sheet open={reductionFormOpen} onOpenChange={setReductionFormOpen}>
        <SheetContent hideCloseButton className="flex flex-col p-0">
          ...
          <div className="flex items-center justify-between p-6 border-b border-gray-100">
            <div>
              <h2 className="text-lg font-semibold text-slate-800">Adauga Reducere Pedeapsa</h2>
      ...
      <Sheet open={zpmFormOpen} onOpenChange={setZpmFormOpen}>
        <SheetContent hideCloseButton className="flex flex-col p-0">
          ...
          <div className="flex items-center justify-between p-6 border-b border-gray-100">
            <div>
              <h2 className="text-lg font-semibold text-slate-800">Adauga ZPM</h2>
              <p className="text-sm text-slate-500 mt-0.5">Zile privilegiate de munca pentru luna selectata</p>
            </div>
            <SheetClose asChild>
              <button className="flex items-center justify-center w-8 h-8 rounded-full text-gray-400 hover:text-gray-600 hover:bg-gray-100 transition-colors">
                <X className="h-4 w-4" strokeWidth={2} />
