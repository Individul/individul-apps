...
const TABS = [
  { id: 'all', label: 'Toate' },
  { id: 'overdue', label: 'Depasite' },
  { id: 'imminent', label: 'Iminente' },
  { id: 'upcoming', label: 'In Curand' },
] as const
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-4 mt-2">
            ...
            <span className="text-xs text-gray-500">
              Data tinta: <span className="font-medium text-slate-700 tabular-nums">{formatDate(alert.target_date)}</span>
            </span>
            {!alert.is_read && (
  ...
  return (
    <div className="flex flex-col items-center justify-center py-20 px-4">
      ...
      <h3 className="text-lg font-semibold text-slate-800 mb-2">
        {isFiltered ? 'Nicio alerta in aceasta categorie' : 'Nicio alerta activa'}
      </h3>

      {/* Sub-message */}
      <p className="text-sm text-slate-400 text-center max-w-sm">
        {isFiltered
          ? 'Incercati sa schimbati categoria sau verificati mai tarziu.'
          : 'Toate termenele sunt sub control. Relaxeaza-te.'}
      </p>
    </div>
  )
}
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            Monitorizare automata a termenelor critice
          </p>
          {unreadCount > 0 && (
            <p className="text-xs text-slate-400 mt-1">
              {unreadCount} {unreadCount === 1 ? 'alerta necitita' : 'alerte necitite'}
            </p>
          )}
        </div>
      ...
      {isLoading ? (
        <div className="flex items-center justify-center py-20">
          <div className="flex items-center gap-2 text-slate-400">
            ...
            <span className="text-sm">Se incarca alertele...</span>
          </div>
        </div>
        ...
        <div>
          ...
          <p className="text-xs text-gray-500 mb-4">
            {filteredAlerts.length} {filteredAlerts.length === 1 ? 'alerta' : 'alerte'}
            {activeTab !== 'all' && ` in categoria "${TABS.find(t => t.id === activeTab)?.label}"`}
          </p>
